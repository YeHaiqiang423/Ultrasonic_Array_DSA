...
module ultrasonic_fpga_top (
    ...
    // 只有当复位按键未按下 且 PLL 输出稳定后，才释放系统复位
    ...
    uart_rx_driver #(
        ...
        .BAUD_RATE(115200)       // ⚠️ 确认串口屏波特率！如果不通，改成 9600 试试
    ) u_uart_rx (
        .clk        (clk_100m),  // 喂入倍频后的 100M 时钟
        .rst_n      (core_rst_n),
        .uart_rxd   (uart_rx),   // 物理引脚，接串口屏的 TX 管脚
    ...
    ultrasonic_top u_core (
        .clk         (clk_100m),     // 注意：这里喂的是倍频后的 100M 时钟！
        .rst_n       (core_rst_n),
